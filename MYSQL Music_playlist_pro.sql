create database music_playlist;
 ...
 -- 2. Which countries have the most Invoices?
      ...
      select max(billing_country) from invoice group by billing_country order by billing_country desc limit 1;
...
-- most money 
   ...
   customer join invoice on customer.customer_id = invoice.customer_id group by customer.customer_id
...
-- listeners. Return your list ordered alphabetically by email starting with A
   ...
   first_name,last_name from customer join invoice on customer.customer_id = invoice.customer_id
...
-- longest songs listed first
   ...
   select name,milliseconds from track where milliseconds >(select avg(milliseconds) as avg_track_length
   from track)
